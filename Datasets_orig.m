...
function oneRangeBin(RB)
    ...
    data = raylrnd(0.4,N,1);
    data = sort(data,'ascend');
    data = sort(data,'ascend');
    minValue = 0; % Or whatever you want.
    maxValue = 10; % Or whatever you want.
    indexesInRange = data > minValue & data < maxValue;
    data = data(indexesInRange);
    x = 0:0.0001:max(data);
...
%     csv_append = [RB Range1 sigma_mle_rayl sigma_mom_rayl sigma_mle_logn sigma_mom_logn mu_mle_logn mu_mom_logn shape_mle_wbl,shape_mom_wbl,scale_mle_wbl,scale_mom_wbl shape_watts_k,shape_mom_k,scale_watts_k,scale_mom_k, modChiSqr_rayl_mle, modChiSqr_rayl_mom, modChiSqr_logn_mle, modChiSqr_logn_mom , modChiSqr_wbl_mle, modChiSqr_wbl_mom , modChiSqr_k_watts, modChiSqr_k_mom,ChiSqr_rayl_mle, ChiSqr_rayl_mom, ChiSqr_logn_mle, ChiSqr_logn_mom , ChiSqr_wbl_mle, ChiSqr_wbl_mom , ChiSqr_k_watts, ChiSqr_k_mom,mean_reflectivity, mean_GIT, mean_HYB, mean_TSC, rad2deg(GrazingAngle), A];
%     dlmwrite('CycleDataset2.csv',csv_append,'delimiter',',','-append');    
end
...
%     plotBest(data);       %plots the best fit of each dsitribution
    ...
    dlmwrite('WindSS_Fynmeet.csv',csv_append,'delimiter',',','-append');    
end
function loopDataset()
    ...
    figure();
    rbins(2:2:end) = [];
    arr_modChiSqr_rayl_mle(2:2:end) = [];
    arr_modChiSqr_rayl_mom(2:2:end) = [];
    arr_modChiSqr_logn_mle(2:2:end) = [];
    arr_modChiSqr_logn_mom(2:2:end) = [];
    arr_modChiSqr_wbl_mle(2:2:end) = [];
    arr_modChiSqr_wbl_mom(2:2:end) = [];
    arr_modChiSqr_k_mom(2:2:end) = [];
    arr_modChiSqr_k_watts(2:2:end) = [];
    
    arr_ChiSqr_rayl_mle(2:2:end) = [];
    arr_ChiSqr_rayl_mom(2:2:end) = [];
    arr_ChiSqr_logn_mle(2:2:end) = [];
    arr_ChiSqr_logn_mom(2:2:end) = [];
    arr_ChiSqr_wbl_mle(2:2:end) = [];
    arr_ChiSqr_wbl_mom(2:2:end) = [];
    arr_ChiSqr_k_mom(2:2:end) = [];
    arr_ChiSqr_k_watts(2:2:end) = [];
    rbins(2:2:end) = [];
    arr_modChiSqr_rayl_mle(2:2:end) = [];
    arr_modChiSqr_rayl_mom(2:2:end) = [];
    arr_modChiSqr_logn_mle(2:2:end) = [];
...
%     plot(rbins,arr_ChiSqr_wbl_mom,'LineStyle','-.','LineWidth',1);
    ...
    plot(rbins,arr_modChiSqr_k_mom,'LineWidth',1,'Color',[0, 0.5, 0],'Marker','.');
...
figure(2)
%Data
[No,edges] = histcounts(data,20, 'Normalization','pdf');
edges = edges(2:end) - (edges(2)-edges(1))/2;
scatter(edges, No,'x','k','LineWidth',0.8);
set(gca,'YScale','log')
hold on;
...
%% Dr H method : Wind determines SS
...
%     hav = 1.6*swh;
%     U = 8.67*hav^(0.4); %assume fully developed sea 
%     
%     
%     %Get adjustment factors Ga, Fu, Gw 
%     a  = (14.4*lambda + 5.5)*((gr*hav)/(lambda));
...
%     hav = (4.52*10^-3)*(U)^(2.5);
%     %Try new thing 
%         %get SS from avg_wind
%         SS = (avg_wind/3.16)^(1.25);
%         hav = 1.6*swh;
%         U = avg_wind;
%     %Get adjustment factors Ga, Fu, Gw 
%     a  = (14.4*lambda + 5.5)*((gr*hav)/(lambda));
